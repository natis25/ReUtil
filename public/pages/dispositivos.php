...
$base_url = 'http://' . $_SERVER['HTTP_HOST'] . '/Reeutil/services/';
$dispositivos_url = $base_url . 'dispositivo/dispositivoApi.php';
$marcas_url = $base_url . 'marca/marcaApi.php';
$tipos_url = $base_url . 'tipo/tipoApi.php';

// 2. Función para hacer requests HTTP robustos
function fetchApiData($url) {
    // Intento con file_get_contents primero
    $context = stream_context_create([
        'http' => [
            'ignore_errors' => true // Para leer respuestas incluso con errores HTTP
        ]
    ]);
    
    $response = @file_get_contents($url, false, $context);
    
    // Fallback con cURL si falla
    if ($response === false) {
        $ch = curl_init();
    ...
    if ($response === false) {
        die("No se pudo conectar al servicio en: $url");
    }
    
    return $response;
}

// 3. Obtener datos de dispositivos
try {
    $api_response = fetchApiData($dispositivos_url);
    $dispositivos = json_decode($api_response, true);
    
    if (json_last_error() !== JSON_ERROR_NONE) {
        die("Error decodificando JSON de dispositivos: " . json_last_error_msg() . 
            "\nRespuesta cruda: " . htmlspecialchars(substr($api_response, 0, 500)));
    }
    
    $dispositivos = is_array($dispositivos) ? $dispositivos : [];
    
    // DEBUG - Mostrar estructura de datos
    echo "<!-- Dispositivos: " . print_r($dispositivos, true) . " -->";
    
} catch (Exception $e) {
    die("Error obteniendo dispositivos: " . $e->getMessage());
}
...
// 5. Depuración final
echo "<!-- Depuración:
- Dispositivos: " . count($dispositivos) . " registros
- Marcas: " . count($marcas) . " registros
- Tipos: " . count($tipos) . " registros
-->";
?>
                            ...
                            <div class="mb-3">
                                ...
                                <select class="form-select" id="marca_id" name="marca_id" required>
                                    ...
                                    <?php foreach ($marcas as $marca): ?>
                                        <option value="<?= $marca['id_marca'] ?>"><?= htmlspecialchars($marca['marca']) ?>
                                        </option>
                                    <?php endforeach; ?>
                                </select>
                            </div>
